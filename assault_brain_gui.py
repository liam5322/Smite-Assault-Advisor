#!/usr/bin/env python3
"""
🎮 SMITE 2 Assault Brain - Professional GUI
Integrates with our existing analysis system
"""
...
try:
    ...
    from optimized_assault_brain import HardwareProfile, VoiceCoach
except ImportError as e:
    print(f"Warning: Could not import analysis components: {e}")
...
class AssaultBrainApp(ctk.CTk):
    ...
    def init_analysis_components(self):
        """Initialize our analysis system"""
        try:
            # Hardware detection
            self.hardware = HardwareProfile.detect()
            logger.info(f"Hardware detected: {self.hardware.performance_tier} tier")
            
            # Analysis components
            self.items_scraper = EnhancedItemsScraper()
            self.data_manager = SmiteDataManager()
            
            # Voice coach with cool voices
            self.voice_coach = VoiceCoach(self.hardware)
            self.setup_voice_personalities()
            
            # Local LLM (optional)
            self.llm_brain = None
            try:
                self.llm_brain = LocalLLMBrain()
            ...
            logger.error(f"Analysis components failed to initialize: {e}")
            self.analysis_ready = False
    
    def setup_voice_personalities(self):
        """Setup cool voice options"""
        if hasattr(self.voice_coach, 'engine') and self.voice_coach.engine:
            voices = self.voice_coach.engine.getProperty('voices')
            
            # Find the best voices
            self.voice_options = {
                'Professional': {'rate': 180, 'voice_id': 0},
                'Casual': {'rate': 200, 'voice_id': 0},
                'Hype': {'rate': 220, 'voice_id': 0},
                'Tactical': {'rate': 160, 'voice_id': 0}
            }
            
            # Try to find different voice types
            for i, voice in enumerate(voices):
                if 'female' in voice.name.lower() and i < len(voices):
                    self.voice_options['Tactical']['voice_id'] = i
                elif 'male' in voice.name.lower() and i < len(voices):
                    self.voice_options['Hype']['voice_id'] = i
    ...
    def create_header(self, parent):
        ...
        subtitle_text = f"Professional Team Analysis & Counter Building • {self.hardware.performance_tier.title()} Mode"
    ...
    def create_home_tab(self):
        ...
        cards_data = [
            ...
            ("📊", "Analyses Today", str(len(self.analysis_history)), "Team compositions analyzed"),
            ("🎯", "Hardware", f"{self.hardware.performance_tier.title()}", f"{self.hardware.ram_gb:.0f}GB RAM"),
            ("💬", "Discord", "Connected" if self.settings.get('discord_webhook') else "Not Set", "Share with team")
        ]
    ...
    def create_settings_tab(self):
        ...
        perf_frame = self.create_settings_section(settings_container, "⚡ Performance")
        
        startup_label = ctk.CTkLabel(perf_frame, text="Start with Windows:", font=("Arial", 14))
        startup_label.grid(row=1, column=0, padx=20, pady=10, sticky="w")
        ...
        self.startup_switch.grid(row=1, column=1, padx=20, pady=10)
...
Local LLM: {'Available' if self.llm_brain else 'Not Available'}"""
    ...
    def perform_analysis(self, team1, team2):
        ...
        try:
            ...
            # Voice coaching
            if self.settings.get('voice_enabled', True):
                self.voice_coach.speak_analysis(result)
            
            # Discord sharing
            if self.settings.get('discord_webhook'):
                self.share_to_discord(result)
    ...
    def share_to_discord(self, analysis):
        """Share analysis to Discord webhook"""
        # This would implement Discord webhook sharing
        pass
        
    def setup_system_tray(self):
        """Setup system tray icon"""
        # This will be implemented for system tray functionality
        pass
    ...
    def on_hotkey_pressed(self):
        ...
        if self.monitoring_active and self.analysis_ready:
            ...
            # This would trigger screen capture and analysis
    ...
    def change_voice_personality(self, personality):
        ...
        # Update voice coach personality
        if hasattr(self.voice_coach, 'personality'):
            self.voice_coach.personality = personality.lower()
    ...
    def update_volume(self, value):
        ...
        # Update voice coach volume
        if hasattr(self.voice_coach, 'engine') and self.voice_coach.engine:
            self.voice_coach.engine.setProperty('volume', int(value) / 100)
    ...
    def toggle_startup(self):
        ...
        self.settings['start_with_windows'] = self.startup_switch.get()
        self.save_settings()
        # Implement actual Windows startup registration here
    ...
    def analyze(self):
        ...
        if result:
            ...
            messagebox.showinfo("Analysis Complete", 
                f"Analysis complete! Win rate: {result['win_probability']*100:.0f}%\n"
                f"Check the Analysis tab for full details.")
